...
class MediaPipeVisionEngine(VisionEngine):
    ...
    def start_capture(self) -> None:
        ...
        self._capture_thread = threading.Thread(
            target=self._capture_loop, daemon=True, name="VisionEngineThread"
        )
    ...
    def _extract_landmarks(
        ...
        for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            for landmark in hand_landmarks.landmark:
                ...
                landmarks_normalized.append({"x": landmark.x, "y": landmark.y, "z": landmark.z})

                # Pixel coordinates
                landmarks.append(
                    {"x": int(landmark.x * width), "y": int(landmark.y * height), "z": landmark.z}
                )
    ...
    def _apply_smoothing(self, landmarks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for curr_hand, prev_hand in zip(landmarks, self._prev_landmarks):
            ...
            for curr_lm, prev_lm in zip(curr_hand["landmarks"], prev_hand["landmarks"]):
                smoothed_lm = {
            ...
            for curr_lm_norm, prev_lm_norm in zip(
                curr_hand["landmarks_normalized"], prev_hand["landmarks_normalized"]
            ):
                smoothed_lm_norm = {
